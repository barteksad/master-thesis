...
\newcommand{\invariantimages}[1]{Invariants}  % Invariant Images
...
% Invariant neuron optimization, we target some neuron, for example taken from semantic leans \cite dreyer2025mechanisticunderstandingvalidationlarge  e.g. this one 1656a: 0.09r.:94.5% "Black and white stripes" where "alignment a to an expected concept (label below concept examples), we provide the highest relevance scores r on the test set of the class." and show that we can generate images that also activate it but look completly not like zebra (here comes some image). Here it is important to get the hypothesis right: " repeat experiment for layer3.2.conv1: [179] with a proper (matched to one of the hypothesized semantics) image / choose simpler neuron with proper image, e.g., layer3.2.conv1: [1656] # Layer 3, "zebra", high relevance 94.5%"

% We do the same for sae from joseph2025prismaopensourcetoolkit and show that some concepts are active on images that look differently

% and of cource we do the same just for res net classifier probability / logit output


% this one is maybe: for SAE: generate images that i) maximize reconstruction error, ii) are semifactuals wrt to the original image implement new optimization scheme with penalty for trajectory closeness and starting from the true image latent

% some additional nice looking metric -  radial power spectrum
% It visualizes the average magnitude of frequency components in an image's 2D Fourier Transform as a function of radial distance from the center (zero frequency). It shows how power is distributed across frequencies, with low frequencies near the center and high frequencies at the edges. In the provided code, the plot compares real and synthetic images:

% X-axis: Radial distance (frequency bins from the center).
% Y-axis: Average magnitude of frequency components.

% As loss
% We can also use this as a loss function but it needs to be differentiable.
% There is implementation in src/loss/radial_spectrum.py but we dont use it
